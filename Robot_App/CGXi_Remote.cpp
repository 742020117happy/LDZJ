...
// 初始化：创建ModBus客户端→移至工作线程→绑定信号槽→启动→1.5s后自动连接
...
// 连接成功：标记状态，启动1000寄存器分批读取(每批124个)
...
// ================= 状态机流转 =================
// 读完成回调：将buffer数据memcpy到全局数组，尝试处理写队列，然后继续下一批读取
...
// 遍历写请求队列，找到第一个待写入寄存器并分派(每次只写一批，读写互斥)
