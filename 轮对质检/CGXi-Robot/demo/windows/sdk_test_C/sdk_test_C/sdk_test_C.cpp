// sdk_test_C.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
void api_demo_cr_script_popup_msg_set()
{
	...
	strcpy_s(invarMsg.variableName, sizeof(name), name);
...
void api_demo_cr_moveJog()
{
	...
	pointControlPara.poseTranType = poseTranMoveToTargetPose;
	pointControlPara.motiontriggerMode = MovetriggerbyOnlyRpc;
	CRresult result = cr_moveJog(robotHandle, pointControlPara);  //点动运动
	assert(result == sucess);
}
...
{
	char filePath[100] = "D:/file/path.crpath";
	PathData pathData;
	pathData.pathPoints = new PathPoint[10000];
...
void api_demo_cr_set_installVarValue()
{
	...
	strcpy_s(installVar.variableName, name);
...
void api_demo_cr_set_ToolOutputVoltage()
{
	...
	toolPower = Power_on;
	CRresult result = cr_set_ToolOutputVoltage(robotHandle, toolPower);
	//设置监控界面的工具输出电压为24v；
	assert(result == sucess);
	printf("%d\n", toolPower);
}
...
{
	enum ToolPower toolPower;
	CRresult result = cr_get_ToolOutputVoltage(robotHandle, &toolPower);
	//读取监控界面的工具输出电压
	assert(result == sucess);
	printf("%d\n", toolPower);
}
...
void api_demo_cr_cfg_tcp_add()
{
	...
	int i;
	char name[20] = { "TCP1" };
	strcpy_s(tcpMsg.tcpName, sizeof(name), name);
...
void api_demo_cr_cfg_payload_add()
{
	...
	strcpy_s(payLoad.payloadName, sizeof(name), name);
...
void api_demo_cr_cfg_safety_plane_add()
{
	...
	strcpy_s(safetyLimitsBoundaryPlane.name, sizeof(name), name);
...
void api_demo_cr_cfg_var_bit_reg_set()
{
	...
	strcpy_s(bitRegisterConfiguration.GeneralPurposeBOOLeanRegisterName, sizeof(Name), Name);
...
void api_demo_cr_cfg_cs_line_add()
{
	...
	strcpy_s(lineCSNode.firstPoint.name, sizeof(point_name1), point_name1);
	strcpy_s(lineCSNode.secondPoint.name, sizeof(point_name2), point_name2);
	strcpy_s(lineCSNode.name, sizeof(name), name);
	for (int i = 0; i < 6; i++)
	{
...
void api_demo_cr_cfg_cs_plane_add()
{
	...
	strcpy_s(planeCSNode.firstPoint.name, sizeof(point_name1), point_name1);
	strcpy_s(planeCSNode.secondPoint.name, sizeof(point_name2), point_name2);
	strcpy_s(planeCSNode.thirdPoint.name, sizeof(point_name3), point_name3);
	strcpy_s(planeCSNode.name, sizeof(name), name);
	for (int i = 0; i < 6; i++)
	{
